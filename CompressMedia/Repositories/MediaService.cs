...
namespace CompressMedia.Repositories
{
	public class MediaService : IMediaService
	{
		...
		/// <returns></returns>
		/// <exception cref="FileNotFoundException"></exception>
		public bool UploadAndCompressMedia(MediaDto mediaDto)
		{
			...
			if (mediaDto.Media != null)
			{
				...
				// OptimizeVideo
				string fileNameOutput = OptimizeVideo("wwwroot" + src, Guid.NewGuid() + mediaDto.Media.FileName);

				string srcOptimized = root + fileNameOutput;
				...
				if (string.IsNullOrEmpty(rootPathOptimized)) return false;
		...
		/// <param name="mediaDto"></param>
		/// <returns></returns>
		public bool UploadMedia(MediaDto mediaDto)
		{
			// Kiểm tra người dùng có đăng nhập chưa
			string cookie = _authService.GetLoginInfoFromCookie();
			string cookieDecode = _authService.DecodeFromBase64(cookie);
